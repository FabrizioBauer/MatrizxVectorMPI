...
void matrizxVector(int mytid, int nproc,int size, MPI_Comm comm){
    ...
    //SI SOY MULTIPLO
        if(size%nproc==0){
            int coc = size/nproc;
            int matrizFilasAsignadas[coc][size];//Si se rompe es porque es coc+1
            for (int i = 0; i < coc; i++){
            ...
            for (int i = 0; i < coc; i++){
                ...
                //printf("Soy el proceso %d valor resParcial %d\n",mytid,resParcial[i] );
            }
            for (int i = 0; i < coc; i++){
        ...
        //SI NO SOY MULTIPLO
            ...
            if(mytid<mod){
                ...
                if(mytid==0){
                    //matrizFilasAsignadas[coc]=A[coc*nproc];SI HAY ERRORES ES ACA |PREGUNTAR
                    for (int i = 0; i < size; i++){
            ...
            //todos hacen esto (esto deberia pasar?) 
            for (int i = 0; i < coc; i++){
                MPI_Gather(&resParcial[i], 1, MPI_INT, &b[i*nproc], 1, MPI_INT, 0, comm);
            }
            //si tengo que hacer trabajo extra Y ACA SE SUPONE QUE FUNCA
            if(mytid<mod){
                if(mytid==0){
